        ...
        for k_atom in range(len(rxyz_new)):
            ...
            for i_atom in range(len(rxyz_new)):
                ...
                for j_atom in range(len(rxyz_new)):
                    ...
                    fp_dist = fp_dist + np.matmul(diff_fp.T, diff_fp)
            ...
            if types[i_atom] == itype:
                for j_atom in range(len(rxyz)):
                    if types[j_atom] == itype:
                        ...
                        diff_fp = fp_iat - fp_jat
                        temp_num = fpdist_error + np.matmul(diff_fp.T, diff_fp)
                ...
                for j_atom in range(len(rxyz)):
                    ...
                    if kat_in_i_sphere_left == True and kat_in_j_sphere_left == True:
                        diff_D_fp_x_left = D_fp_mat_iat_left[0, :, kat_i_left] \
                                         - D_fp_mat_jat_left[0, :, kat_j_left]
                        diff_D_fp_y_left = D_fp_mat_iat_left[1, :, kat_i_left] \
                                         - D_fp_mat_jat_left[1, :, kat_j_left]
                        diff_D_fp_z_left = D_fp_mat_iat_left[2, :, kat_i_left] \
                                         - D_fp_mat_jat_left[2, :, kat_j_left]
                    elif kat_in_i_sphere_left == True and kat_in_j_sphere_left == False:
                        diff_D_fp_x_left = D_fp_mat_iat_left[0, :, kat_i_left]
                        diff_D_fp_y_left = D_fp_mat_iat_left[1, :, kat_i_left]
                        diff_D_fp_z_left = D_fp_mat_iat_left[2, :, kat_i_left]
                    elif kat_in_i_sphere_left == False and kat_in_j_sphere_left == True:
                        diff_D_fp_x_left = - D_fp_mat_jat_left[0, :, kat_j_left]
                        diff_D_fp_y_left = - D_fp_mat_jat_left[1, :, kat_j_left]
                        diff_D_fp_z_left = - D_fp_mat_jat_left[2, :, kat_j_left]
                    else:
                        diff_D_fp_x_left = np.zeros_like(D_fp_mat_jat_left[0, :, kat_j_left])
                    ...
                    if kat_in_i_sphere_right == True and kat_in_j_sphere_right == True:
                        diff_D_fp_x_right = D_fp_mat_iat_right[0, :, kat_i_right] \
                                          - D_fp_mat_jat_right[0, :, kat_j_right]
                        diff_D_fp_y_right = D_fp_mat_iat_right[1, :, kat_i_right] \
                                          - D_fp_mat_jat_right[1, :, kat_j_right]
                        diff_D_fp_z_right = D_fp_mat_iat_right[2, :, kat_i_right] \
                                          - D_fp_mat_jat_right[2, :, kat_j_right]
                    elif kat_in_i_sphere_right == True and kat_in_j_sphere_right == False:
                        diff_D_fp_x_right = D_fp_mat_iat_right[0, :, kat_i_right]
                        diff_D_fp_y_right = D_fp_mat_iat_right[1, :, kat_i_right]
                        diff_D_fp_z_right = D_fp_mat_iat_right[2, :, kat_i_right]
                    elif kat_in_i_sphere_right == False and kat_in_j_sphere_right == True:
                        diff_D_fp_x_right = - D_fp_mat_jat_right[0, :, kat_j_right]
                        diff_D_fp_y_right = - D_fp_mat_jat_right[1, :, kat_j_right]
                        diff_D_fp_z_right = - D_fp_mat_jat_right[2, :, kat_j_right]
                    else:
                        diff_D_fp_x_right = np.zeros_like(D_fp_mat_jat_right[0, :, kat_j_right])
                        diff_D_fp_y_right = np.zeros_like(D_fp_mat_jat_right[1, :, kat_j_right])
                        diff_D_fp_z_right = np.zeros_like(D_fp_mat_jat_right[2, :, kat_j_right])

                    diff_D_fp_x_left = np.vstack( (np.array(diff_D_fp_x_left), ) ).T
                    diff_D_fp_y_left = np.vstack( (np.array(diff_D_fp_y_left), ) ).T
                    diff_D_fp_z_left = np.vstack( (np.array(diff_D_fp_z_left), ) ).T
                    diff_D_fp_x = np.vstack( (np.array(diff_D_fp_x), ) ).T
                    diff_D_fp_y = np.vstack( (np.array(diff_D_fp_y), ) ).T
                    diff_D_fp_z = np.vstack( (np.array(diff_D_fp_z), ) ).T
                    diff_D_fp_x_right = np.vstack( (np.array(diff_D_fp_x_right), ) ).T
                    diff_D_fp_y_right = np.vstack( (np.array(diff_D_fp_y_right), ) ).T
                    diff_D_fp_z_right = np.vstack( (np.array(diff_D_fp_z_right), ) ).T
